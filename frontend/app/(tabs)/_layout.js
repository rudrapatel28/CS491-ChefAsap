...
export default function TabLayout() {
    const { isAuthenticated, userType, isLoading, profileId, token, isGuestBrowsing } = useAuth();
    ...
    useEffect(() => {
        if (isGuestBrowsing || userType === 'guest') return;
    ...
    }, [isLoading, isAuthenticated, router, isGuestBrowsing, profileId, apiUrl, token, userType]);
    ...
    if (isGuestBrowsing || userType === 'guest') {
        return (
    ...
    return (
        <View style={{ flex: 1, backgroundColor: CREAM }}>
                    ...
                    {bookings.length > 0 && (
                        userType === 'chef' ? (
                            <OrderConfirmationModal
                                ...
                                onClose={() => setBookings(bookings.filter((booking) => booking.booking_id !== bookings[0].booking_id))}
                                booking={bookings[0]}
                            />
                        ) : (
                            <BookingReviewModal
            ...
            <Tabs screenOptions={tabBarOptions}>
                ...
                />
                <Tabs.Screen name="Messages" options={messagesTabOptions} />
                <Tabs.Screen
                    name="Profile"
                    options={{
                        title: 'Profile',
