...
export default function ChefMenu() {
    ...
    const [guestPaymentMethod, setGuestPaymentMethod] = useState(null);
    ...
    const createGuestAccount = async () => {
        ...
        try {
            const res = await fetch(`${apiUrl}/guest`, {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                Alert.alert(
                    ...
                    data.error || 'Could not create guest account'
                );
                return null;
            }
            ...
            Alert.alert('Error', 'Could not create guest account');
            return null;
        }
    };
    ...
    const handlePlaceOrder = async () => {
        let guestId = guestToken;
        let guestPaymentId = null;
        if (userType === 'guest') {
            ...
            if (
                ...
                Alert.alert(
                    ...
                    'Please enter your first name, last name, and either an email address or phone number.'
                );
                return;
            }
            ...
            if (userType === 'guest') {
                ...
                setGuestPaymentMethod(paymentMethod);

                guestPaymentId = paymentMethod.id;
            }
            ...
            try {
                ...
                const bookRes = await fetch(`${apiUrl}/booking/create`, {
                    ...
                    body: JSON.stringify({
                        customer_id: userType === 'guest' ? null : profileId,
                        guest_id: userType === 'guest' ? guestId : null, cuisine_type: orderItems[0]?.cuisine_type || 'Mixed',
                                ...
                                <View style={st.modalCard}>
                                    ...
                                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
                                            ...
                                            {userType === 'guest' && (
                                                ...
                                                <>
                                                    ...
                                                    <TextInput
                                                        ...
                                                        autoCapitalize="none" value={guestInfo.email}
                                                        onChangeText={(v) =>
                                                            setGuestInfo({
                                                                ...guestInfo,
                                                                email: v
                                            ...
                                            {userType !== 'guest' && (
                                                ...
                                                paymentMethods.map(method => (
                                                    <TouchableOpacity
                                                        ...
                                                        style={[
                                                            st.paymentRow,
                                                            selectedPaymentMethod === method.id && st.paymentRowSelected
                                                        ]}                                                >
                                        ...
                                        <View style={{ flexDirection: 'row', gap: 10, marginTop: 4 }}>
                                            ...
                                            <TouchableOpacity
                                                style={[st.placeOrderBtn, { flex: 1, marginTop: 0 }, (
                                                    paymentProcessing ||
                                                    (userType !== 'guest' && !paymentMethods.length) ||
                                                    mealConflicts.length > 0
                                                ) && { backgroundColor: '#c8ddd0' }]}
                                                onPress={handlePlaceOrder}
                                                disabled={
                                                    paymentProcessing ||
                                                    (userType !== 'guest' && !selectedPaymentMethod) ||
                                                    (userType === 'guest' && !guestCard?.complete) ||
                                                    mealConflicts.length > 0
                                                }
                                                activeOpacity={0.85}
                                            >
